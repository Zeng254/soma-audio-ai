...
class Reverb(BaseEffect):
    ...
    def _freeverb_process(self, audio: np.ndarray) -> np.ndarray:
        """
        Freeverb style reverb processing
        
        This is Schroeder model optimized implementation
        """
        # Comb filter count
        num_combs = 8
        num_allpasses = 4
        
        # Comb filter delay time (samples)
        comb_tunings = [1116, 1188, 1277, 1356, 1422, 1491, 1617, 1557]
        allpass_tunings = [556, 441, 341, 225]
        
        # Adjust to current sample rate
